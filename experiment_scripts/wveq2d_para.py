...
class AcousticTomographyDataset:
    ...
    def __init__(self, L=1.0, T=2.0, c=1.0, nl=20, nt=26, frequency=3.0, snr_db=20.0, pulse_width=0.1):
        ...
        self.source_x = 0.0
        self.source_x_coords = torch.linspace(-self.L / 2, self.L / 2, self.nl).float()
    ...
    def generate_projection_data(self, beam_res=50):
        ...
        for i in range(self.nl):
            src_x = self.source_x_coords[i]
            det_x = self.detector_x_coords[i]
            L_v_params[idx:idx + self.nt, 0] = det_x
            Ps = torch.tensor([src_x.item(), self.source_y])
...
def calculate_projection_integral_from_nn(model, L_v_params, T_v, dataset, beam_res=50):
    ...
    # --- 平行ビーム座標計算 (P(s) = (x_beam, y_start + s * L_beam)) ---
    
    # ビームの開始Y座標、終了Y座標
    Ps_y = dataset.source_y
    Pd_y = dataset.detector_y
    
    # ビームのX座標 (L_v_paramsから取得)
    # (N_data, 1) -> (N_data, beam_res) -> (N_data * beam_res, 1)
    Beam_x_rep = L_v_params[:, 0].unsqueeze(1).repeat(1, beam_res).view(-1, 1)

    # L_beam (ビーム長) の計算 (全ビームで共通)
    L_beam = Pd_y - Ps_y
    
    # L_beam_rep を (N_data * beam_res, 1) の形状で作成
    L_beam_rep = torch.full_like(Beam_x_rep, L_beam)

    # 積分パス上のパラメータ s を L_beam_rep (ビーム長) にスケール
    s_path = s_points_rep * L_beam_rep 
    
    # 経路上の X, Y 座標を計算
    X_coords = Beam_x_rep # X座標はビームのX座標 (Beam_x_rep) で一定
    Y_coords = Ps_y + s_path # Y座標は Ps_y から s_path 分だけ進む
    ...
    # ※平行ビームでは L_beam_rep は全ビームで共通だが、元のコード構造を維持
...
def evaluate_and_plot(model, dataset, T_v_full, V_v_full, sl=100, save_dir=None, show_plots=True):
    ...
    ax3[0].set_ylim(0, max(np.mean(nmse_series) * 3, 1.0))
    ax3[0].grid(True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--experiment", 
                        type=str, 
                        default="SamuelExperiment_Parallel", 
