...
class AcousticTomographyDataset:
    ...
    def __init__(self, L=1.0, T=2.0, c=1.0, nl=20, nt=26, frequency=3.0, snr_db=20.0, pulse_width=0.1):
        ...
        # --- 中心通過・回転ビーム ジオメトリ (トモグラフィー) ---
        # nl=20 を「回転角度の数」として扱う
        
        self.n_angles = self.nl
        print(f"回転ビーム設定: {self.n_angles} 角度 (すべて中心 (0,0) を通過)")

        # 角度 (0° から 180° 未満)
        self.angles = torch.tensor(np.linspace(0.0, np.pi, self.n_angles, endpoint=False)).float()
        
        # ビームの始点・終点を計算するため、ドメインより十分大きな長さを定義
        self.beam_length = self.L * 1.5 
        
        # (古い source_x, source_y, detector_y, detector_x_coords は削除)
    ...
    def generate_projection_data(self, beam_res=50):
        ...
        L_v_params = torch.zeros(N_data, 2) # 角度 (theta) を格納
        
        idx = 0
        # Ps = torch.tensor([self.source_x, self.source_y]) # ← 削除
        
        for i in range(self.n_angles):
            theta = self.angles[i]
            
            # ビームの方向ベクトル V_norm (角度thetaと同じ)
            # (cos(theta), sin(theta))
            V_norm_x = torch.cos(theta).item()
            V_norm_y = torch.sin(theta).item()

            # ビームの中心点 C は常に (0, 0)
            Center_x = 0.0
            Center_y = 0.0
            
            # ビームの始点 Ps = C - V_norm * (L/2)
            Ps_x = Center_x - V_norm_x * (self.beam_length / 2.0)
            Ps_y = Center_y - V_norm_y * (self.beam_length / 2.0)
            
            # ビームの終点 Pd = C + V_norm * (L/2)
            # (今回は使わないが、参考のために定義)
            # Pd_x = Center_x + V_norm_x * (self.beam_length / 2.0)
            # Pd_y = Center_y + V_norm_y * (self.beam_length / 2.0)
            
            # L_v_params に (theta) を保存
            L_v_params[idx:idx + self.nt, 0] = theta.item()

            # 積分パス上の座標を計算
            s_path = s_points * self.beam_length # s_points は [0, 1]
            X_coords = Ps_x + s_path * V_norm_x
            Y_coords = Ps_y + s_path * V_norm_y
            ds_step = self.beam_length / (beam_res - 1)
...
def visualize_synogram_comparison(dataset, V_v_noisy, V_v_true):
    ...
    detector_angle_labels = dataset.angles.cpu().numpy()
    ...
    im1 = ax1.imshow(
        ...
        # ↓↓↓↓ extent の中身を修正 ↓↓↓↓
        extent=[detector_angle_labels.min(), detector_angle_labels.max(), time_labels.max(), time_labels.min()],
        vmin=v_min, vmax=v_max
    )
    ax1.set_title('True Signal (Noiseless)')
    # ↓↓↓↓ X軸ラベルを修正 ↓↓↓↓
    ax1.set_xlabel(f'Beam Angle (radians)')
    ...
    # ↓↓↓↓ X軸ラベルを修正 ↓↓↓↓
    ax2.set_xlabel(f'Beam Angle (radians)')
...
def visualize_projection_path(dataset, beam_index=0, t=0.0):
    ...
    # beam_index から 角度 (theta) を特定
    i = beam_index
    theta = dataset.angles[i]
    ...
    # ビームの始点 Ps と終点 Pd
    Ps_x = Center_x - V_norm_x * (dataset.beam_length / 2.0)
    Ps_y = Center_y - V_norm_y * (dataset.beam_length / 2.0)
    Pd_x = Center_x + V_norm_x * (dataset.beam_length / 2.0)
    Pd_y = Center_y + V_norm_y * (dataset.beam_length / 2.0)

    Ps = torch.tensor([Ps_x, Ps_y])
    Pd = torch.tensor([Pd_x, Pd_y])

    # 積分パス上の X, Y 座標
    s_path = s_points * dataset.beam_length
    X_coords = Ps_x + s_path * V_norm_x
    Y_coords = Ps_y + s_path * V_norm_y
    ...
    ax.scatter(X_coords.numpy(), Y_coords.numpy(), c='red', s=30, label=f'Sampling Points (Beam {beam_index}, $\\theta$={theta.item():.2f})', zorder=3)
